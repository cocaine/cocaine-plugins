...
namespace {
struct create_handler_t : public unicorn::writable_adapter_base_t<api::unicorn_t::response::create> {
...
{
    filter_list_subscription_t(logging_t::impl_t& _parent) :
        parent(_parent)
    { }

    virtual void
    write(std::tuple<cocaine::unicorn::version_t, std::vector<std::string>>&& filter_ids);
...
struct id_subscription_t : public unicorn::writable_adapter_base_t<api::unicorn_t::response::subscribe> {
    id_subscription_t(logging_t::impl_t& _parent, uint64_t _id) :
        parent(_parent),
        id(_id)
    { }

    virtual void
    write(api::unicorn_t::response::subscribe&& filter_value);
...
struct logging_t::impl_t {
        ...
        list_scope()
    {
        ...
        scopes.unsafe()[id] = unicorn->create(std::make_shared<create_handler_t>(*this, id), filter_unicorn_path, unicorn::value_t(), false, false);
        list_scope = unicorn->children_subscribe(std::make_shared<filter_list_subscription_t>(*this), filter_unicorn_path);
    }
    ...
    logging::filter_t::id_type
    set_filter(const std::string& name, logging::filter_t filter, uint64_t ttl, logging::filter_t::disposition_t disposition) {
        logging::filter_t::deadline_t deadline(std::chrono::steady_clock::now() + std::chrono::seconds(ttl));
        ...
        if(disposition == logging::filter_t::disposition_t::local) {
            auto metafilter = metafilters.apply([&](std::map<std::string, std::shared_ptr<logging::metafilter_t>>& mf) {
                auto& ret = mf[info.logger_name];
                if (ret == nullptr) {
                    std::unique_ptr<logging::logger_t> mf_logger(new blackhole::wrapper_t(*internal_logger, {{"metafilter", name}}));
            ...
            scopes.apply([&](std::unordered_map<size_t, api::unicorn_scope_ptr>& _scopes){
                _scopes[scope_id] = unicorn->create(handler, filter_unicorn_path + format("/%llu", id), info.representation(), false, false);
            });
        } else {
            BOOST_ASSERT(false);
        }
    ...
    bool
    remove_filter(logging::filter_t::id_type id) {
    ...
    list_filters_visitor_t::storage_type
    list_filters() {
        return metafilters.apply([&](std::map<std::string, std::shared_ptr<logging::metafilter_t>>& mfs){
    ...
    void listen_for_filter(logging::filter_t::id_type id) {
        ...
        scopes.apply([&](std::unordered_map<size_t, api::unicorn_scope_ptr>& _scopes){
            _scopes[scope_id] = unicorn->subscribe(handler, filter_unicorn_path + format("/%llu", id));
        });
    }
...
void
create_handler_t::abort(const std::error_code& ec, const std::string& reason) {
    COCAINE_LOG_ERROR(parent.internal_logger, "failed to create filter in unicorn, {} - {}", reason, ec.message());
...
void
filter_list_subscription_t::write(std::tuple<cocaine::unicorn::version_t, std::vector<std::string>>&& filter_ids) {
    COCAINE_LOG_DEBUG(parent.internal_logger, "received filter list update, count - {}", std::get<1>(filter_ids).size());
...
void
filter_list_subscription_t::abort(const std::error_code& ec, const std::string& error) {
    COCAINE_LOG_ERROR(parent.internal_logger, "failed to receive filter list update, {} - {}", error, ec.message());
    //TODO : timer
...
void
id_subscription_t::write(api::unicorn_t::response::subscribe&& filter_value) {
    ...
    try {
        if(filter_value.get_version() == unicorn::not_existing_version) {
            ...
            [&](std::map<std::string, std::shared_ptr<logging::metafilter_t>>& mf) {
                ...
                if (ret == nullptr) {
                    std::unique_ptr<logging::logger_t> internal_logger(
                    new blackhole::wrapper_t(*parent.internal_logger, {{"metafilter", info.logger_name}}));
        ...
        COCAINE_LOG_ERROR(parent.internal_logger, "can not create cluster filter - %s", error::to_string(e));
    }
}

void
id_subscription_t::abort(const std::error_code& ec, const std::string& error) {
    COCAINE_LOG_ERROR(parent.internal_logger, "failed to receive filter update, {} - {}", error, ec.message());
}



class logging_t::logging_slot_t :
    public io::basic_slot<io::base_log::get>
{
public:
    ...
    logging_slot_t(logging_t& _parent) :
        parent(_parent)
    { }

    virtual
    boost::optional<std::shared_ptr<const dispatch_type>>
    operator()(tuple_type&& args, upstream_type&&) {
        return tuple::invoke(std::forward<tuple_type>(args), [&](std::string name) {
            auto metafilter = parent.impl->metafilters.apply(
                [&](std::map<std::string, std::shared_ptr<logging::metafilter_t>>& metafilters) {
                    ...
                    if(mf == nullptr) {
                        std::unique_ptr<logging::logger_t> internal_logger(new blackhole::wrapper_t(*(parent.impl->internal_logger), {{"metafilter", name}}));
                        mf = std::make_shared<logging::metafilter_t>(std::move(internal_logger));
                    }
                    return mf;
                }
            );
            std::shared_ptr<const dispatch_type> dispatch = std::make_shared<const named_logging_t>(parent.impl->logger, std::move(name), std::move(metafilter));
...
struct dynamic_visitor_t {
    ...
    operator()(const dynamic_t::array_t&) {
        throw std::system_error(std::make_error_code(std::errc::invalid_argument), "can not process array as attribute value");
    }

    void
    operator()(const dynamic_t::object_t&) {
        throw std::system_error(std::make_error_code(std::errc::invalid_argument), "can not process map as attribute value");
    }

    void
    operator()(dynamic_t::null_t) {
        throw std::system_error(std::make_error_code(std::errc::invalid_argument), "can not process null as attribute value");
    }
...
void
logging_t::deleter_t::operator()(impl_t* ptr) {
    delete ptr;
};
...
logging_t::logging_t(context_t& context, asio::io_service& asio, const std::string& service_name, const dynamic_t& service_args) :
    ...
    impl(new impl_t(
        ...
        [&]() {
            ...
            return registry.builder<blackhole::config::json_t>(stream).build(service_args.as_object().at("backend", "core").as_string());
        }(),
    ...
    ))
{
    ...
    on<io::base_log::set_filter>(std::bind(&impl_t::set_filter, impl.get(), ph::_1, ph::_2, ph::_3, logging::filter_t::disposition_t::local));
    ...
    on<io::base_log::set_cluster_filter>(std::bind(&impl_t::set_filter, impl.get(), ph::_1, ph::_2, ph::_3, logging::filter_t::disposition_t::cluster));
}

named_logging_t::named_logging_t(logging::logger_t& _log, std::string _name, std::shared_ptr<logging::metafilter_t> _filter) :
    dispatch<io::named_log_tag>(std::move(_name)),
    log(_log),
    filter(std::move(_filter))
{
    on<io::named_log::emit>(std::bind(&named_logging_t::emit, this, ph::_1, ph::_2, ph::_3));
    on<io::named_log::emit_ack>(std::bind(&named_logging_t::emit_ack, this, ph::_1, ph::_2, ph::_3));
}

void
named_logging_t::emit(const std::string& message, unsigned int severity, const logging::attributes_t& attributes) {
    emit_ack(message, severity, attributes);
}

bool
named_logging_t::emit_ack(const std::string& message, unsigned int severity, const logging::attributes_t& attributes) {
