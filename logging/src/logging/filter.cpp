...
{
    if(!value.is_object()) {
        throw cocaine::error_t("invalid representation for filter info - %s", boost::lexical_cast<std::string>(value));
    }
    const auto& obj = value.as_object();
    if(!obj.count("filter") || !obj.count("deadline") || !obj.count("id") || ! obj.count("logger_name")) {
        throw cocaine::error_t("invalid representation for filter info - %s", boost::lexical_cast<std::string>(value));
    }
...
public:
    ...
    virtual
    filter_result_t
    apply(const std::string& message, unsigned int severity, const logging::attributes_t& attributes) const = 0;
...
struct filter_creation_visitor_t {
    ...
    result_type
    operator()(T&&) {
        throw std::system_error(std::make_error_code(std::errc::function_not_supported), "invalid representation - cannot create filter from value");
    }
...
struct null_filter_t: public filter_t::inner_t {
    ...
    apply(const std::string&, unsigned int, const logging::attributes_t&) const {
        throw std::system_error(std::make_error_code(std::errc::operation_not_supported), "tried to apply null filter");
    }

    virtual
    dynamic_t
    representation() const {
        throw std::system_error(std::make_error_code(std::errc::operation_not_supported), "tried to get representation of null filter");
    };
};
...
struct exists_filter_t: public filter_t::inner_t {
    ...
    virtual
    filter_result_t
    apply(const std::string&, unsigned int, const logging::attributes_t& attributes) const {
        return static_cast<bool>(logging::find_attribute(attributes, attribute_name))
               ? fr::accept
               : fr::reject;
    }
...
struct equals_filter_t: public comparision_filter_t<T> {
    ...
    apply(const std::string&, unsigned int, const logging::attributes_t& attributes) const {
        ...
        return (val && val->value.template convertible_to<T>() && val->value.template to<T>() == this->attribute_value)
                ? fr::accept
                : fr::reject;
    }
    ...
    dynamic_t
    representation() const {
        return dynamic_t(std::make_tuple(std::string("=="), this->attribute_name, this->attribute_value));
    };
};
...
struct or_filter_t: public filter_t::inner_t {
    ...
    or_filter_t(filter_t f1, filter_t f2):
        filter1(std::move(f1)),
        filter2(std::move(f2))
    {}
    ...
    apply(const std::string& message, unsigned int severity, const logging::attributes_t& attributes) const {
        return (filter1.apply(message, severity, attributes) == fr::accept || filter2.apply(message, severity, attributes) == fr::accept)
                ? fr::accept
                : fr::reject;
    }
    ...
    dynamic_t
    representation() const {
        return dynamic_t(std::make_tuple(std::string("||"), filter1.representation(), filter2.representation()));
    };
};
...
struct and_filter_t: public filter_t::inner_t {
    ...
    and_filter_t(filter_t f1, filter_t f2):
        filter1(std::move(f1)),
        filter2(std::move(f2))
    {}
...
struct xor_filter_t: public filter_t::inner_t {
    ...
    xor_filter_t(filter_t f1, filter_t f2):
        filter1(std::move(f1)),
        filter2(std::move(f2))
    {}
    ...
    filter_result_t
    apply(const std::string& message, unsigned int severity, const logging::attributes_t& attributes) const {
        return filter1.apply(message, severity, attributes) != filter2.apply(message, severity, attributes)
                ? fr::accept
                : fr::reject;
    }
    ...
    dynamic_t
    representation() const {
        return dynamic_t(std::make_tuple(std::string("xor"), filter1.representation(), filter2.representation()));
    };
};
...
filter_t::filter_t(const dynamic_t& representation) {
    if(!representation.is_array()) {
        throw error_t("representation should be array, found - %s", boost::lexical_cast<std::string>(representation));
    }
